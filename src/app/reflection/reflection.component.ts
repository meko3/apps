...
export class ReflectionComponent implements OnInit {
  ...
  public checkAll() {
    let checkFlag = true;
    if (this.myValues.length === this.values.length) {
      this.values.forEach((value, index) => {
        if (!this.myValues[index].data) {
          checkFlag = false;
  ...
  public initPoints() {
    this.myValues.forEach((value) => {
      this.myValuePoints[value.tag] = value.point;
    });
  }

  public checkRank(select?) {
    if (this.checkCombinationFlag()) {
      if (select === 0) {
        this.myValuePoints[this.leftValue]++;
        this.myValuePoints[this.rightValue]--;
      }
      if (select === 1) {
        this.myValuePoints[this.leftValue]--;
        this.myValuePoints[this.rightValue]++;
      }
      this.setCombination(this.combinationFlag);
      this.combinationFlag++;
    }
  }

  public setCombination(index) {
    this.leftValue = this.combination[index][0];
    this.rightValue = this.combination[index][1];
  }
  ...
  public sortValues() {
    this.rankedValues = [];
    this.myValuePoints.forEach((value, index) => {
      this.checker++;
      this.rankedValues.push([index, value]);
    });
    this.rankedValues.sort((pre, cur) => {
      return cur[1] - pre[1];
    });
  }
